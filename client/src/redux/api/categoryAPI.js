...
export const remove = ( id) => {
  return async (dispatch) => {
    dispatch(categoryActions.setError(null));
    await request
      .delete(`/api/categorys/${id}`,{ headers: {
...
export const update = (id,data, cb, message = false) => {
  return async (dispatch) => {
    dispatch(categoryActions.setError(null));
    await request
      .put(`/api/categorys/${id}`, data,{ headers: {
        token: `${JSON.parse(localStorage.getItem("userInfo")).token}`,
      }})
      .then((res) => {
        dispatch(categoryActions.setError(null));
